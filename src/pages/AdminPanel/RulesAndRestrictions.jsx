...
function Restrictions() {
  const [items, setItems] = useState([]); const [form, setForm] = useState(emptyRestriction); const [query, setQuery] = useState(""); const [search, setSearch] = useState(""); const [error, setError] = useState(""); const [saving, setSaving] = useState(false);
  const load = useCallback(async () => { try { setError(""); setItems((await api.get("/admin/restrictions", { params: { search: search || undefined } })).data.items); } catch (requestError) { setError(requestError.response?.data?.error || "No se pudieron consultar las restricciones"); } }, [search]);
  useEffect(() => { void load(); }, [load]);
