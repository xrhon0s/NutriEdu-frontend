...
El panel usa una navegacion lateral persistente en escritorio y un selector compacto en mobile. La seccion Operacion muestra metricas agregadas, cobertura de perfiles, presupuesto de IA, ledger de migraciones y cinco indicadores accionables de calidad del catalogo. Estos indicadores enlazan con recetas o ingredientes para completar nutrientes, revision, cantidades, grupos alimentarios y grupos de sustitucion. Usuarios incorpora busqueda, filtro, paginacion, tarjetas mobile, cobertura clinica y cambios de rol con confirmacion explicita; el backend vuelve a validar cada cambio y protege a la ultima cuenta administradora. Catalogos clinicos permite buscar, paginar, crear, editar, activar y desactivar objetivos y condiciones, mostrando riesgo y orientacion profesional. Reglas y restricciones ofrecen formularios contextuales, unidades derivadas del nutriente, filtros de alcance/estado, paginacion y desactivacion no destructiva. Uso de IA muestra resumen filtrado, tasa de exito, solicitudes, tokens, costos, fallos y codigos de error, con la politica de limites en modo de solo lectura. Todos los listados administrativos adoptan tarjetas mobile y tablas de columnas prioritarias. Estas operaciones requieren JWT y rol administrativo verificado en backend.
...
El formulario administrativo de recetas consulta ingredientes en paginas de 12, permite buscarlos y conserva la selección al cambiar de pagina. Cada ingrediente seleccionado admite cantidad, unidad y equivalencia en gramos; los dos primeros alimentan Compras y los gramos habilitan calculos nutricionales reproducibles. Tambien captura nutrientes por porcion, tamano de porcion, numero de porciones y procedencia; una fuente revisada exige su referencia y registra al administrador responsable. Los listados administrativos de recetas e ingredientes usan tarjetas informativas en mobile y tablas de columnas prioritarias desde tablet; comparten busqueda, estados, paginacion y confirmaciones accesibles. La migracion backend `010_recipe_nutrition_provenance.sql` requerida por estos campos esta aplicada y verificada en Supabase.

Cuando todos los ingredientes seleccionados tienen gramos, el formulario puede solicitar un calculo por porcion al backend. Solo se habilita si los perfiles por 100 g estan completos y revisados. El resultado llena los ocho nutrientes, porcion y procedencia calculada; cualquier edicion posterior de gramos, porciones o nutrientes invalida esa procedencia.
